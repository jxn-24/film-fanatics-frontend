...
const FilmfanaticsProfile = () => {
  ...
  console.log('Current user from Redux:', user); // Debug user object
  ...
  useEffect(() => {
    ...
    console.log('Profile.jsx useEffect triggered, user:', user);
  ...
  const ChatModal = ({ targetId, onClose }) => {
    ...
    useEffect(() => {
      ...
      if (targetId) {
        ...
        console.log(`Fetching messages for user ${user?.id} and target ${targetId}`);
    ...
    const handleSend = () => {
      ...
      console.log(`Sending message to ${targetId}: ${input}`);
    ...
    if (user && user.token) {
      ...
      try {
        ...
        const mappedData = {
          ...
          // avatar is not handled by backend currently
        ...
        setProfileData(data); // Update with backend response
